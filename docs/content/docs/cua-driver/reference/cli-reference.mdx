...
- `--client <name>` (optional): one of `claude`, `codex`, `cursor`. Omit for the generic JSON snippet that any MCP client accepts.
...
# claude mcp add --transport stdio cua-driver -- /Applications/CuaDriver.app/Contents/MacOS/cua-driver mcp

cua-driver mcp-config --client codex
# codex mcp add cua-driver -- /Applications/CuaDriver.app/Contents/MacOS/cua-driver mcp

cua-driver mcp-config --client cursor
# JSON snippet to paste into ~/.cursor/mcp.json (Cursor has no CLI)
```

Pipe to `pbcopy` to put on the clipboard, or use the printed `claude` / `codex` commands directly.
