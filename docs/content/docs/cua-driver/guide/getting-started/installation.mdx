...
| Codex | `~/.agents/skills/cua-driver` | when `~/.codex/` exists (creates the agents dir if missing) |

The symlink survives auto-updates because `/Applications/CuaDriver.app` is replaced atomically — the link target stays valid across releases.

For agents without a SKILL.md auto-loader (Cursor, OpenClaw, OpenCode, Hermes, Pi), point them at the bundled skill manually if you want the same instructions:

```bash
cat /Applications/CuaDriver.app/Contents/Resources/Skills/cua-driver/SKILL.md
```

You can paste it into the agent's system prompt, instructions file, or whatever per-project context mechanism the agent uses (e.g. Cursor's `.cursorrules`, project-level `AGENTS.md`).
