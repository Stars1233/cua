...
Cua Driver speaks MCP over stdio. Pick the command for your client:
...
Add `--scope project` to register only for the current repo, or `--scope global` for every project.
...
Cursor has no CLI for MCP — paste the JSON into `~/.cursor/mcp.json` (or your client's equivalent):

```bash
cua-driver mcp-config --client cursor | pbcopy
```

For any other client that accepts the standard `mcpServers` shape:
